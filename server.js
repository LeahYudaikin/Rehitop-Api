...
// הגדרת נתיבים לפי הרצה רגילה או pkg
...
const getNextId = () => {
  ...
  try {
    ...
    console.error("Error reading idFile:", error);
  }
...
app.post('/products/upload-image', (req, res) => {
  ...
  upload(req, res, (err) => {
    ...
    res.json({ imagePath: `${folderPath}/${req.file.filename}` });
  });
});

// מחיקת תמונה
app.post('/products/delete-image', authenticateToken, (req, res) => {
  const imagePath = req.body.imagePath.replace(`http://localhost:${port}/`, '');
  if (!imagePath) {
...
app.post('/products', authenticateToken, (req, res) => {
  ...
  if (!newProduct.name) newProduct.name = newProduct.Id;
...
app.put('/products/:id', authenticateToken, (req, res) => {
  ...
  if (!updatedProduct.name) updatedProduct.name = updatedProduct.Id;
...
// כל הבקשות הלא מוכרות יפנו ל-Angular
app.get('*', (req, res) => {
